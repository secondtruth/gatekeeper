...
class SpambotsBlacklistCheck implements CheckInterface
{
    ...
    public function checkVisitor(Visitor $visitor)
    {
        ...
        foreach ($this->getSpambotNamesBeginning() as $spambot) {
            $pos = strpos($uastring, $spambot);
            if ($pos !== false && $pos == 0) {
                return '17f4e8c8';
            }
        }

        foreach ($this->getSpambotNamesAnywhere() as $spambot) {
            if (strpos($uastring, $spambot) !== false) {
                return '17f4e8c8';
            }
        }

        foreach ($this->getSpambotNamesRegex() as $spambot) {
            if (preg_match($spambot, $uastring)) {
                return '17f4e8c8';
            }
        }

        foreach ($this->getSpambotUrls() as $spambot) {
